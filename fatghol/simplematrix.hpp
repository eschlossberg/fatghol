...
private:
  ...
  _simplerows m;
#else
  typedef Givaro::QField<Givaro::Rational> _CoefficientRingType;
  typedef LinBox::SparseMatrix<_CoefficientRingType, LinBox::SparseMatrixFormat::SparseSeq> _MatrixType;
...
is_null_product(const SimpleMatrix &A, const SimpleMatrix &B)
{
  ...
  for(unsigned int i=0; i<A.m.rowdim(); i++) {
    for (unsigned int j=0; j<B.m.coldim(); j++) {
      SimpleMatrix::_CoefficientRingType::Element x;
      SimpleMatrix::_CoefficientRingType::Element y;
      ZZ.init(x, 0);
      ZZ.init(y, 0);
      for (unsigned int k=0; k<A.m.coldim(); k++)
        x += A.m.getEntry(i,k) * B.m.getEntry(k,j);
      if (not (x == y))
        return false;
    }
