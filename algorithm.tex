...
a certain order; for each different choice, we get a different "Fatgraph"
instance representing the same fatgraph.
...
\begin{lemma}
  ...
  representations, and, conversely, every triple "(pv, rot, pe)"
...
The general isomorphism problem for (ordinary) graphs is NP-complete
one\FIXME{Verificare!! E aggiungere referenza!}  However, the
situation is much simpler for fatgraphs, because of the following
property.
\begin{lemma}[Rigidity Property]\label{lemma:rigidity}
  ...
  homotopy fixing the vertices of $G$) by its restriction to $V$.
\end{lemma}
...
known, and the displacement (relative to the cyclic order at $w$) of
the image of the cilium of $v$ with respect to the cilium of the image
vertex $w$.
\begin{proof}
  Let $w = f(v)$; then, $W = f(U)$ is a small neighborhood of $w$.
  It is clear that $V \setminus \{v\}$
\end{proof}
