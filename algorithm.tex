...
\end{lstlisting}
Function "compatible_vertex_mappings" iterates over (indices of)
...
that extend map "pv" in the neighborhood of vertices "v1" (in the
domain) and "v2" (in the codomain).  It returns a list of triplets
"(src, dst, rot)", where:
\begin{itemize}
\item "src" is the index of a vertex in "G1",
   connected to "v1" by an edge "x";
\item "dst" is the index of a vertex in "G2",
   connected to "v2" by the image (according to "pe")
   of edge "x";
\item "rot" is the rotation to be applied to "G1[src]"
   so that edge "x" and its image appear
   at the same index position;
\end{itemize}
The algorithm is implemented by looping over the edges at vertex "v1"
(line~\nr{neighbors:1}): for each edge "x", the variable "src_v"
stores the vertex index of the endpoint of "x" different from "v1"
(line~\nr{neighbors:3}), and "src_a" stores the attachment index of
"x" at this other vertex (line~\nr{neighbors:5}). Looping edges are
...
The body of the "Fatgraph.isomorphism" function begins here.
...
the form $(v_0, w)$ where $v_0$ is a chosen vertex in $G_1$.  It
should be selected so to minimize the number of mapping attempts
performed; this is currently done by minimizing the product of valence
and number of vertices of that valence on $G_2$ (line~\nr{iso:1}), and
then picking a random vertex of the chosen valence in $G_1$ as $v_0$
...
\begin{lstlisting}
    ...
    v0_index = vs1[valence][0]                            |\n{iso:2}|
    v0 = G1.vertices[v0_index]
\end{lstlisting}
Now the algorithm tries all possible ways of mapping $v_0$ into a
compatible vertex $w$ in $G_2$:
\begin{lstlisting}
    for w_index in compatible_vertex_mappings(v0, G2, indexes):
      for rot in range(valence):
        try:
\end{lstlisting}
The main part of the algorithm mimicks the construction in the proof
of Lemma~\ref{lemma:rigidity}.  First, given a target vertex "w" and a
rotation "rot", a new triple "(pv,rots,pe)" is created and set to
represent the initial mapping of (a small neighborhood of) $v_0$ to (a
...
\begin{lstlisting}
          ...
          pv[v0_index] = w_index
          rots[v0_index] = rot
          if not pe.extend(v0, G2.vertices[w_index][rot:rot+valence]):
            continue # to next `rot`

\end{lstlisting}
The invocation to "pe.extend" may fail if the edge labels on $v_0$ and
$w$ (rotated by "rot") do not match, in which case execution jumps to
the next candidate "rot".
Elements in "rots" are initialized to "None", to mark that
the triple only defines a ``partial map''.

The map defined by "(pv,rots,pe)" is then extended to neighbors of the
vertices already inserted:
\begin{lstlisting}
          nexts = neighbors(pv, pe, G1, v0_index, G2, w_index)   |\n{iso:3}|
...
Indeed:
\begin{itemize}
\item the variable "nexts" is loaded with a list of all possible
  mappings of a vertex adjacent to $v_0$ onto a vertex adjacent to $w$
  (line~\nr{iso:3});
\item if any such mapping can extend the current "(pv,rots,pe)", the
  neighbors of the newly-added vertex pair are appended to the
  "neighborhood" list (lines~\nr{iso:4}--\nr{5});
\item the contents of "neighborhood" are used to load the variable
  "nexts" during the subsequent iteration:
\end{itemize}

If any "extend_map" failed in the above block, continue with next
candidate "rot":
\begin{lstlisting}
...
isomorphism and we can return it to the caller function:
\begin{lstlisting}
...
"isomorphism" method: if invariants do not compare equal, then the two
fatgraphs are not isomorphic and we can bail out early:
\begin{lstlisting}
    if self is other:
      return True
...
actual isomorphism, we just try getting one (line~\nr{fgeq:1}): if we
succed, the isomorphisms exists and we return a positive answer to the
equality question; else, Python signals a "StopIteration" exception to
indicate that there are no results from the "isomorphism" iterator
(line~\nr{fgeq:2}): so there are no isomorphisms between the given
graphs and we can only return "False":
\begin{lstlisting}
    try:
