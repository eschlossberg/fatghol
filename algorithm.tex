...
unique $\nu_2$ such that "s" can be non-null;\FIXME{Quindi,
  all'interno di un blocco, su ogni colonna c'\'e un solo indice
  nonzero.  Questa potrebbe essere una osservazione importante per il
  calcolo del rango: per\`o non sono riuscito finora ad applicarla in
  alcun modo.} by Lemma~\ref{lemma:sigma+aut}, there exist
$\nu_2^{(k)} = \sigma_2^{(k)} \circ \bar\nu_2$ and $a \in \Aut G$ such
that $(G_2, \nu_2^{(k)})$ is representative the isomorphism class
$[G_2,\nu_2]$ stored within the "NumberedFatgraphPool" instance "p2",
and $a$ gives the isomorphism between numbered fatgraphs $(G_2,\nu_2)$
and $(G_2, \nu_2^{(k)})$.  Actual computation of these data is the
purpose of the final part of this function.
