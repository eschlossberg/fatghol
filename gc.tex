\RCSID $Id: gc.tex,v 1.4 2006/05/12 11:00:59 rmurri Exp $
%auto-ignore
...
\end{theorem}%
\nx{Z_\A}{Reshetikhin-Turaev graphical calculus functor on the
  category $\A$}
...
\end{definition}%
\nx{$\Gamma$}{Generic ribbon graph}%
\nx{$\In(\Gamma)$}{Input legs of graph $\Gamma$}%
\nx{$\Out(\Gamma)$}{Output legs of graph $\Gamma$}%
\nx{$\RG$}{Category of ribbon graphs}%
\nx{$\RG(p,q)$}{Linear span of ribbon graphs of type $(p,q)$}%
...
\nx{$\Vertices{\Gamma}$, $\Edges{\Gamma}$, $\Holes{\Gamma}$}{Sets of vertices,
  edges, and boundary components of the ribbon graph $\Gamma$}%
