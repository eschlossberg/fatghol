...
as a relative of Harer's arc-system complex \cite{%
  harer;cohomological-dimension,%
  harer;cohomology-of-moduli%
},\FIXME{%
  Non sono affatto sicuro che gli arc-system siano stati
  introdotti da Harer.  Per\`o non \`e molto importante chi li ha
  introdotti, quanto che qui si costruisce il complesso dei grafi a
  nastro parallelamente a come Harer costruisce quello degli
  arc-system.}
...
direct approach to constructing a ribbon graph complex by defining
...
$X - X_\infty$, as will be shown in \csref{sec:cell-structure}.
...
detailed in \cite{conant-vogtmann;2003}. Very recently, Hamilton and
...
\begin{proof}
  ...
  each oriented edge $\alpha^\pm \in \beta$, let $T_{\alpha^\pm}^{(\beta)}$ be the curvilinear
  triangle delimited by $\alpha^\pm$ and the radii joining endpoints of $\alpha^\pm$
  with the center of $B_\beta$.  The collection $\{ T_{\alpha^\pm}^{(\beta)} \}$ is a
  ...
  $\alpha'$, $\alpha''$: then $B_\beta$ breaks up into triangles $T_{\alpha'}$ and
...
\end{lemma}
Denote by $\RG$ the category of numbered ribbon graphs and their
morphisms.  By lemmas~\ref{lemma:contraction1}
and~\ref{lemma:contraction2}, $\RG$ decomposes into the disjoint union
of sub-categories $\RG_{g,n}$ whose objects are ribbon graphs of genus
$g$ with $n$ punctures:
\begin{equation*}
...
\begin{definition}\label{dfn:embedded-rg}
  ...
  of $\Diff_0(S)$, of triples $[G, \iota, \{\psi_i\}_{i=1}^n]$ composed of a numbered
  ribbon graph $G$, an embedding $\iota:G\to S$, and, for each connected
  component $C_i$ of $S - \iota(G)$, a map $\psi_i: {\bar C_i} \to {\bar B}^\times$
  such that
  \begin{inparaenum}
...
Embedded numbered ribbon graphs with their morphisms form a category
$\ERG$, which is again disjoint union of categories $\ERG_{g,n}$.
There is an obvious forgetful functor from the category of embedded
ribbon graphs to the category of abstract numbered ribbon graphs,
...
ribbon graph complex and (co)homology of $\M_{g,n}$.  
...
\begin{remark}
  ...
  face is identified with the simplex $|\langle u_1, \ldots, \widehat{u_j}, \ldots,
  u_k\rangle|$.
\end{remark}
...
\begin{lemma}
  ...
  $U \in X - X_\infty$ and embedded ribbon graphs ${\tilde G} \in \ERG_{g,n}$,
...
\begin{proof}
  ...
  symplex $|U| \subseteq |X - X_\infty|$; by the proof of Harer's \csref{thm:H}
  ...
  of $S_{g,n} - u$ is homeomorphic to one of the open sets $V_\gamma$ (with
...
\begin{lemma}
  ...
  in $X$ with a face on $X_\infty$; every loop in a ribbon graph ${\tilde G}$
  corresponds to a face on $X_\infty$ in the dual arc-system.
\end{lemma}
\begin{proof}
  Let ${\tilde G}$ be an embedded ribbon graph.  If no edge of
  ${\tilde G}$ is a loop, then any contraction of an edge in ${\tilde
    G}$ produces an embedded ribbon graph in $\ERG_{g,n}$, which again
  ...
  Conversely, suppose that $U' \in X_\infty$ is such that adding a single arc
  $u_0$ to $U'$ determines a simplex belonging to $X - X_\infty$.  Let
  ...
  \item The arc $u_0$ is entirely contained in a connected component
    $B$ of $S - u'$, and this $B$ is either a punctured disk or a disk
    with a slit in the middle.  Indeed, let $B' := B - |u_0| = S - U'
    - u_0$; since $\langle u_0, U'\rangle$ belongs to $X - X_\infty$, then $B'$ is a
    topological disk.  The border of $B'$ is union of the arcs $u_i$,
    where each arc occurs at most two times, and, for those occurring
    twice, the two copies have equal length in the metric of $S$.  The
    arc $u_0$ occurs two times on the border of $B'$, and we get $B$
    by isometrically gluing $B'$ along the two copies of $u_0$.
...
\begin{lemma}
  ...
  \item The above correspondence is compatible with the face
    operations: if $U' \in X - X_\infty$ is a face of $U \in X - X_\infty$, then
  ...
  \end{inparaenum}
\end{lemma}
By \csref{lemma:PL-face}, the face operators on the two set of
...
If $G$ is a ribbon graph with $p$ edges, let $W_G := \bigwedge^p \setQ\Edges{G}$
...
\begin{definition}
  The complex $(W_*, D)$ of orientable ribbon graphs is defined by:
  \begin{itemize}
  \item $W_p := \bigoplus_G W_G$, where $G$ runs over orientable ribbon graphs
    with $(2g + n - 1 + p)$ edges;
...
Every oriented ribbon graph $(G, \omega)$ defines an element $\omega_G \in W_G$ by
...
\begin{proof}
  ...
  brevity, put $\Gamma := \Gamma_{g,n}$, $\T := \T\_{g,n}$ and $\Tcomb :=
  ...
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
  Let $R'_p$ be the collection of all orientable ribbon graphs with
  ...
  \cong \bigoplus_{G \in R_p} W_G$.  Forming the border of a cell $T{\tilde G} \subseteq
