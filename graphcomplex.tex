...
The route to it is a bit convoluted, as the direct approach to
constructing a ribbon graph complex by defining face operators through
contraction of edges does not yield a simplicial complex: after a few
contractions, loops appear, which cannot be contracted any more.  This
mirrors the structure of Harer's $U \setminus U_0$, as will be shown in
\csref{sec:cell-structure}.
...
graph complex, complete with full details, is given in
\cite{conant-vogtmann;2003}, closely following Kontsevich' original
work, or in \cite{hamilton-lazarev;math.QA/0608395}, where a new proof
is given.
...
theorem, the two approaches turn out to be the same.
...
also been called ``fatgraphs'' in the literature.
...
contracting an edge $\alpha \in \Edges{G}$ to a point.  If $\alpha$ connects twop
\emph{distinct} vertices, that is, $\alpha$ is not a loop, then $G'$
inherits a ribbon graph structure from $G$: if $(\alpha < \alpha_1 < \ldots < \alpha_k <
...
\subsection{From ribbon graphs to Riemann surfaces, I}
\label{sec:rg-to-surfaces}

There is a functorial construction to build a topological surface
$S(G)$ from a ribbon graph $G$; this is usually referred to as
...
\begin{lemma}
  ...
  \begin{inparaenum}
  \item There is a functor $S$ that associates to every ribbon graph
...
\end{lemma}
\begin{proof}
  Let $G = (L; \sigma_0, \sigma_1, \sigma_2)$ be a ribbon graph.  Associate a 0-cell
  with every $\sigma_0$-orbit in $L$, and a 1-cell with every $\sigma_1$-orbit;
  attach a 1-cell $\alpha = \{\alpha^+, \alpha^-\}$ to the (possibly not distinct) 0-cells
  corresponding to the $\sigma_0$-orbits of $\alpha^+$ and $\alpha^-$.  Now glue a
  punctured 2-cell $B_\beta^\times$ along any $\sigma_2$-orbit $\beta$: if $\{l, \sigma_2(l), \ldots,
  ...
  punctured disk $B_\beta^\times$ has been glued to each $\sigma_2$-orbit $\beta$; for
  each oriented edge $\alpha^\pm \in \beta$, let $T_\alpha^{(\beta)}$ be the curvilinear
  triangle delimited by $\alpha^\pm$ and the radii joining endpoints of $\alpha^\pm$
  with the center of $B_\beta$.  The collection $\{ T_{\alpha} \}$ is a
  triangulation of $S(G)$.  When $G$ has a loop $\alpha$, there is a
  $\sigma_2$-orbit $\beta$ such that $\beta$ is made of $\alpha$ alone, thus $B_\beta$ is
  bounded by $\alpha$ only.  Split $\alpha$ at the midpoint to form two edges
  $\alpha'$, $\alpha''$: then $B_\beta$ breaks up in the triangles $T_{\alpha'}$ and
  $T_{\alpha''}$ and we can proceed as before.
...
\subsection{Numbered ribbon graphs}
...
\end{definition}
From this point onwards, \emph{all ribbon graphs are equipped with a
  numbering}.
...
\begin{definition}\label{dfn:embedded-rg}
  An embedded ribbon graph is an equivalence class, modulo the action
  of $\Diff_0(S)$, of triples $[G, \iota, \{\psi_i\}_{i=1}^n]$ composed of a
  ribbon graph $G$, an embedding $\iota:G\to S$, and, for each connected
  component $C_i$ of $S \setminus \iota(G)$, a map $\psi_i: {\bar C_i} \to {\bar B}^\times$
  such that
  \begin{inparaenum}
...
surface $S$ (modulo the action of $\Diff^0(S)$).\FIXME{%
  ...
  \ref{dfn:embedded-rg} come caratterizzazione?}%
Since a morphism $f:G\to G'$ of abstract ribbon graphs induces
to a morphism $S(f):S(G)\to S(G')$, then a morphism of embedded
...
The mapping class group $\Gamma_{g,n}$ acts functorially on $\ERG_{g,n}$ by
...
\begin{itemize}
\item for any edge $\alpha \in \Edges{G}$, let $V_\alpha := (T_\alpha^+ \cup T_\alpha^-)^\circ$ be
  the interior of the union of the two triangles having $\alpha$ as an
  edge;
...
\begin{definition}
  ...
  A metric $\ell$ on a ribbon graph $G$ is an assignment of a real
...
\begin{lemma}
  ...
  If $G$, $G'$ are metrized ribbon graphs and $f$ is an isomorphism
...
construction of ribbon graphs from smooth complex curves.
...
additional structure of \emph{numbered} graph.
...
Let $G$ be a ribbon graph of genus $g$ with $n$ numbered boundary
components.  A topological simplex is spanned when varying the metric
data $(\ell_\alpha)_{\alpha \in \Edges{G}}$; gluing these cells one recovers the
whole $\T_{g,n}$ (when using \emph{embedded} ribbon graphs) or
$\M_{g,n}$ (when using \emph{abstract} ribbon graphs). $\M_{g,n}$.  
A construction of $\Tcomb_{g,n}$ and $\Mcomb_{g,n}$can be found in
\cite{penner:math.GT/0210326}, although the procedure outlined here
extends the construction of $\Mcomb_{g,n}$ given in
...
\begin{definition}
  ...
  functor $T$:\FIXME{%
    Penner costruisce questo spazio non come limite diretto, ma
    incollando esplicitamente le celle.  C'\`e qualcosa di nascosto che
    non funziona col limite diretto?  Mi sembra di no.}%
  \begin{equation*}
...
\begin{lemma}
  ...
  the abstract ribbon graph $G$ underlying ${\tilde G}$.
\end{lemma}
\begin{proof}
  ...
  critical graph ${\tilde G}$.  Yet, since $q$ has critical graph
  ${\tilde G}$, $\tau^*q$ has critical graph $\tau({\tilde G})$. Therefore,
  $\tau$ restricts to a ribbon graph isomorphism, so a map $\Gamma_{{\tilde
...
The cells $T({\tilde G})$ (resp.~$M(G)$) have real dimension
...
recast in terms of ribbon graphs, provides the needed link between the
ribbon graph complex and (co)homology of $\M_{g,n}$.  The route to it
is a bit convoluted, as the direct approach to constructing a ribbon
graph complex by defining face operators through admissible
contractions does not yield a simplicial complex: after a few
contractions, loops appear, which cannot be contracted any more.
...
simplex $\{ t_* \in \setR^{n+1} | t_0 + \cdots + t_n = 1 \}$.
...
$S \setminus u$, and an edge for every edge of $u$.  Corresponding edges of
...
\begin{lemma}
  ...
  $u \in U \setminus U_0$ and embedded ribbon graphs ${\tilde G} \in \ERG[g,n]$,
  the graph corresponding to an arc-system $u$ being the dual graph to
  $u$ in the ambient surface.
\end{lemma}
\begin{proof}
  Let $u = [u_1, \ldots, u_k] \in U \setminus U_0$ be an arc-system on the Riemann
  surface $S_{g,n}$.  Any choice of barycentric coordinates $b_0 + \cdots +
  b_k = 1$ in $\Delta(u)$ identifies a point in the associated geometric
  symplex $|u| \subseteq |U \setminus U_0|$; by the proof of Harer's \csref{thm:H}
  ...
  \emph{embedded} ribbon graph.  By construction, ${\tilde G}$ is the
  dual graph of $u$.
  ...
  differential $\tau^*q$, which has ${\tilde G}$ as critical graph.

  Conversely, let ${\tilde G}$ be a ribbon graph embedded in the
  surface $S_{g,n} = S(G)$.  Break up $S_{g,n}$ into closed triangles
  $\{ T_\alpha \}$ as in \csref{lemma:S-functor}(b).  Form the dual graph to
  ${\tilde G}$: for each $\{ T_{\alpha^\pm}$, let $u_\alpha^\pm$ be the arc
  connecting the midpoint of $\alpha \in \Edges{{\tilde G}}$ to the vertex of
  $T_{\alpha^\pm}$ opposite to $\alpha$, and let $u_\alpha$ be the arc obtained by
  joining $u_\alpha^\pm$ at their common endpoint.  Then $u = [u_{\alpha_1}, \ldots,
  u_{\alpha_k}]$ is a filling arc-system: indeed, each connected component
  of $S_{g,n} \setminus u$ is homeomorphic to one of the open sets $V_\gamma$ (with
  $\gamma \in \Vertices{G}$), therefore it is a disk.
\end{proof}
...
\begin{lemma}
  \label{lemma:II}
  \begin{inparaenum}
  \item There is a canonical PL isomorphism between $|u|$ and
    $T({\tilde G}) = \Delta(G)$.
  \item The above correspondence is compatible with the face
    operations: if $u' \in U \setminus U_0$ is a face of $u \in U\setminus U_0$, then
    the dual graph ${\tilde G}'$ is obtained from ${\tilde G}$ by
    contraction of a non-loop edge, and the PL isomorphism $|u|
    \isoarrow T({\tilde G})$ restricts to $|u'| \isoarrow T({\tilde
      G}')$ on the faces.
  \end{inparaenum}
\end{lemma}
\begin{proof}
  Recall that the cells that make up $\Tcomb_{g,n}$ are indexed by
  embedded ribbon graphs, and let ${\tilde G} \in \ERG[g,n]$ be an
  embedded ribbon graph. If no edge of ${\tilde G}$ is a loop, then
  any contraction of an edge in ${\tilde G}$ produces an embedded
  ribbon graph in $\ERG[g,n]$, which again corresponds to a simplex in
  $U \setminus U_0$.  Therefore, embedded ribbon graphs with no loops identify
  cells in $U$ with no face on $U_0$.
  ...
  correspondingly, if $u'$ be the face of $u$ obtained by omitting the
  ...
  ${\tilde G}$ be the embedded ribbon graph, dual to $\langle u_0, u'\rangle$.
  It's simple to see then:
  \begin{enumerate}
  \item The arc $u_0$ is entirely contained in a connected component
    $B$ of $T \setminus |u'|$, and this $B$ is either a punctured disk or a
    disk with a slit in the middle.  Indeed, let $B' := B \setminus |u_0|$;
    since $\langle u_0, u'\rangle$ belongs to $U \setminus U_0$, then $B'$ is a
    topological disk.  The border of $B'$ is union of the arcs $u_i$,
    where each arc occurs at most two times, and, for those that occur
    two times, the two copies have equal length in the metric of $T$.
    The arc $u_0$ occurs two times on the border of $B'$, and we get
    $B$ by isometrically gluing $B'$ along the two copies of $u_0$.
...
By \csref{lemma:approaching-face}, the face operators on the two set
of simplices are compatible with this PL isomorphism, so we can glue
isomorphisms $|\langle u_*\rangle| \to |G|$ to form a PL isomorphism $|U \setminus U_0| \to
\Tcomb_{g,n}$.
...
\begin{definition}\label{dfn:orientation}
  An orientation of a ribbon graph $G$ is an orientation of the vector
  space $\setQ\Edges{G}$, that is, the choice of an order of the edges of
  $G$, up to an even number of exchanges.
\end{definition}
Giving an orientation on $G$ (resp.~${\tilde G}$) is the same as
orienting the simplex $\Delta(G)$ (resp.~$T({\tilde G}$).

If $G$ is a ribbon graph with $p$ edges, let $\Lambda_G := \bigwedge^p \setQ\Edges{G}$
...
\land \alpha_p \mapsto f(\alpha_1) \land \ldots \land f(\alpha_p)$.  It is trivial to check that $f_*(\alpha_1 \land
...
Form a differential complex of orientable graphs as follows.
\begin{definition}
  The complex $(\Lambda_*, D)$ of orientable ribbon graphs is defined by:
  \begin{itemize}
  \item $\Lambda_p := \bigoplus_G \Lambda_G$, where $G$ runs over orientable ribbon graphs
    with $(2g + n - 1 + p)$ edges;
  \item $D: \Lambda_p \to \Lambda_{p-1}$ is given by $D := \sum_1^p (-1)^i d_i$ where
    \begin{equation*}
      d_i(\alpha_1 \land \ldots \land \alpha_p) := \alpha_1 \land \ldots \land \widehat{\,\alpha_i\,} \land \ldots \land \alpha_p,
    \end{equation*}
    iff $\alpha_i$ is not a loop and $G/\alpha_i$ is orientable, otherwise set 
    \begin{equation*}
      d_i(\alpha_1 \land \ldots \land \alpha_p) := 0.
    \end{equation*}
  \end{itemize}
\end{definition}
Every oriented ribbon graph $(G, \omega)$ defines an element $\omega_G \in \Lambda_G$ by
taking the wedge product of edges of $G$ in the order given by $\omega$;
conversely, any $\alpha_1 \land \ldots \land \alpha_p \in \Lambda_G$ defines an orientation on $G$ by
setting $\alpha_1 < \ldots < \alpha_p$.

\begin{theorem}
  The $\Gamma_{g,n}$-equivariant homology of $\T_{g,n}$ with rational
  coefficients is computed by the complex of oriented ribbon graphs
  $(\Lambda_*, D)$, that is, there exists an isomorphism
  \begin{equation*}
    H^{\Gamma_{g,n}}_*(\T_{g,n}, \setQ) \cong H_*(\Lambda_*, D)
  \end{equation*}
\end{theorem}
\begin{proof}
  The genus and puncutre number will be fixed throughout, so for
  brevity, put $\Gamma := \Gamma_{g,n}$, $\T := \_{g,n}$ and $\Tcomb :=
  \Tcomb_{g,n}$.  By \csref{thm:HMT}, we have:
  \begin{equation*}
  ...
  cellularization with cells indexed by embedded ribbon graphs of
  ...
  $\Gamma$.  By \csref{lemma:penner-kontsevich-bridge}, $R_p$ is in
  bijective correspondence with the set of abstract ribbon graphs
  ...
  \csref{lemma:isotropy}, there is an isomorphism between $\Gamma_{\tilde
  ...
  $G$.  Therefore, $\setQ{\tilde G}$ and $\Lambda_G$ are isomorphic as $\Aut G =
  \Gamma_{T{\tilde G}}$ modules.

  Following \cite[p.~173]{brown}, decompose (as a $\Gamma$-module)
  $C_*(\Tcomb, \setQ) = \bigoplus_{G \in R_p} \Lambda_G$; then, by Shapiro's lemma
  \cite[III.6.2]{brown}, we have:
  \begin{equation*}
  ...
  $H_q(\Aut G, \Lambda_G) = 0$ if $q > 0$ \cite[III.10.2]{brown}.  On the
  other hand,
  \begin{equation*}
    H_0(\Aut G, \Lambda_G) =
    \begin{cases}
      ...
      &\text{if $G$ has \emph{no} orientation-reversing automorphisms%
        \footnote{In other words: $G$ is orientable, so $\Aut G$ acts
          trivially on $\Lambda_G$.}}
    \end{cases}
  \end{equation*}
  Hence, substituting back into the spectral sequence,
  \begin{align}
    \label{eq:11}
    E^1_{p,0} &= \Lambda_p,
    &&
    ...
    &\text{for all $q > 0$,}
  \end{align}
  where $R'_p$ is the collection of all orientable ribbon graphs with
  $p$ edges.

  Finally, we show that the differential $d^1: E^1_{p,0} \to E^1_{p-1,
    0}$ corresponds to the differential $D: \Lambda_p \to \Lambda_{p-1}$ under the
  isomorphism \csref{eq:11}; this will end the proof.  Indeed, we
  ...
  \begin{equation}
    ...
    \xymatrix{%
      P_* \otimes \Lambda_p = 
      &
      \bigoplus_{G \in R'_p} P_* \otimes \Lambda_G
      &
      \bigoplus_{G' \in R'_{p-1}} P_* \otimes \Lambda_{G'} 
      & 
      = P_* \otimes \Lambda_{p-1} 
      \\
      & 
      ...
      \ar_{\theta_p} "1,2";"2,2"
      \ar^{\theta_{p-3}} "1,3";"2,3"
    }
  \end{equation}
  which implies commutativity at the homology level:
  \begin{equation*}
    \xymatrix{%
      \bigoplus_{G \in R'_p} H_0(\Aut G, \Lambda_G)
      &
      \bigoplus_{G' \in R'_{p-1}} H_0(\Aut G', \Lambda_{G'})
      \\
  ...
  \end{equation*}
  whence the thesis $E^1_{*,0} \cong (\Lambda_*, D)$.
  ...
  \cong \bigoplus_{G \in R_p} \Lambda_G$.  Forming the border of a cell $T{\tilde G} \subseteq
  ...
  ${\tilde G}$ iff the underlying \emph{abstract} ribbon graphs $G'$
  and $G$ stand in the same relation.  Thus, the $\Gamma$-complexes $(C_p,
  \partial)$ and $(\bigoplus\Lambda_G, D)$ are isomorphic by $\theta_*$, so \eqref{eq:13}
